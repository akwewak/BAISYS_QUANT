...
class DataValidator:
    ...
    def validate_dataframe_not_empty(
        ...
        if df is None:
            ...
            if self.logger:
                self.logger.warning(f"[数据验证] {error_msg}")
            raise ValueError(error_msg)
        
        if not isinstance(df, pd.DataFrame):
            error_msg = f"{data_name} 不是 DataFrame 类型，实际类型: {type(df)}"
            if self.logger:
                self.logger.error(f"[数据验证] {error_msg}")
    ...
    def validate_stock_code_format(
        ...
        if fixed_count > 0 and self.logger:
            self.logger.info(f"[数据验证] {data_name}: 修复了 {fixed_count} 个股票代码格式")
        ...
        try:
            ...
            if expected_date:
                try:
                    ...
                    if self.logger:
                        self.logger.warning(f"[数据验证] {data_name}: 日期解析失败 - {e}")
    ...
    def comprehensive_validate(
        ...
        # 2. 必需列验证
        if required_cols:
            cols_valid, missing_cols = self.validate_required_columns(
                df, required_cols, data_name
            )
        ...
        # 3. 价格数据验证
        if price_cols:
            price_valid, anomaly_counts = self.validate_price_data(
                df, price_cols, data_name
            )
        ...
        if check_completeness:
            completeness_valid, null_ratios = self.validate_data_completeness(
...
def get_validator(logger: Optional[LoggerManager] = None) -> DataValidator:
    """
    获取全局数据验证器实例
    ...
    if _global_validator is None:
        _global_validator = DataValidator(logger)
